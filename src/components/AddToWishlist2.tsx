...
const AddToWishList2 = ({ product , className  } : { product : Product , className ?: string }) => {
  ...
  return (
    <div className={cn(``, className)} >
      <div className={`p-2.5  hoverEffect hover:bg-green-950 cursor-pointer   rounded-full hover:text-white text-black hoverEffect ${existingProduct ? "bg-green-950  text-white" : "bg-gray-100"}`}  >
        <Heart size={17} />
      </div>
    </div>
  )
